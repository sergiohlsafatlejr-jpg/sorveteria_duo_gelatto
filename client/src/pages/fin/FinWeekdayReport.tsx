...
export default function FinWeekdayReport() {
  ...
  const { data: summaries = [], isLoading } = trpc.fin.weekdayReport.payablesByWeekday.useQuery(
    { year, month },
  );

  const chartData = summaries.map((s) => ({
    name: s.dayName.split("-")[0].trim(), // "Segunda", "Terça", etc.
            ...
            <div className="divide-y">
              {summaries.map((day) => (
                <div key={day.dayIndex}>
                  ...
                  {expandedDay === day.dayIndex && day.items.length > 0 && (
